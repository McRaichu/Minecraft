...
public class ItemReconBike extends Item {
	public static String unl_name = "item_reconbike";
	
	public ItemReconBike()
    {
        this.maxStackSize = 1;
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    ReconBike entityHarvester = new ReconBike(worldIn, (double)((float)blockpos.getX() + 0.5F), (double)((float)blockpos.getY() + 1.0F), (double)((float)blockpos.getZ() + 0.5F));
                    entityHarvester.rotationYaw = (float)(((MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);
                    //entityboat.rotationYaw = (float)(((MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!worldIn.getCollidingBoundingBoxes(entityHarvester, entityHarvester.getEntityBoundingBox().expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return itemStackIn;
                    }
                    ...
                    if (!worldIn.isRemote)
                    {
                        worldIn.spawnEntityInWorld(entityHarvester);
                    }
